---
### [OK] LOCATE-CONTROL | 2026-03-16
- **Status**: [OK] ADOPTED
- **Objective**: Add "Where Am I?" locate button to the map, enabled by default
- **Hypothesis**: Users need a quick way to find their position on the map
- **Approach**: Custom L.Control with SVG crosshair icon, uses Leaflet's map.locate() API, dispatches CustomEvents
- **Result**:
    - Control: crosshair button in top-left, below zoom controls
    - Visual feedback: blue (locating), green (found), red (error, resets after 2s)
    - Location display: accuracy circle + center dot (blue CircleMarker)
    - Config: `locate: true` by default, disable with `{ locate: false }`
    - Events: `locate:found` (latlng, accuracy, bounds), `locate:error` (message)
    - Tests: 56 unit (4 new) + 54 e2e (7 new), all pass
    - Build: 6.17 KB gzipped UMD (was 5.28 KB, +~900 bytes)
    - Verified: button renders correctly in browser via Rodney
    - Outcome: Success
- **The Delta**: First UI control added by hyperleaflet beyond Leaflet defaults
- **Next Step**: Consider data-attribute toggle (data-locate="false" on map div)
---
...
- **Result**:
  ...
  - Artifacts removed: **pycache**, \*.db files untracked
